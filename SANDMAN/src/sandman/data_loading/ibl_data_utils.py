...
def load_trials_and_mask(
    ...
    if sess_loader is None:
        sess_loader = SessionLoader(one=one, eid=eid) # fixed the missing one=
...
def get_spike_data_per_interval(
    ...
    # doesn't work on Mac due to multiprocessing issues
    ...
    for interval in tqdm(intervals):
        res = compute_spike_count(interval)
        binned_spikes[res[-1], res[1], :] += res[0]
...
def load_anytime_behaviors(one, eid, n_workers=os.cpu_count()):
    ...
    # behave_dict = {} # Doesn't work on Mac due to multiprocessing issues
    # with multiprocessing.Pool(processes=n_workers) as p:
    ...
    for beh in tqdm(behaviors):
        _, result = load_beh(beh)
        behave_dict[beh] = result
