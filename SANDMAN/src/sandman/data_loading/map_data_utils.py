...
def get_trial_spikes_and_licks(hit_left_trials, hit_right_trials, left_lick, right_lick, jaw, nose, tongue, units, ids_sort_by_area, sample_start, delay_start, delay_end, if_opto = False, opto_start = None, opto_type = None):
    ...
    for trials_of_interest, trial_type in zip([hit_left_trials, hit_right_trials], ['left', 'right']):
        for trial_id in trials_of_interest.trial_uid:
            ...
            #breakpoint()
            if (len(sample_start_in_trial) > 1) or (len(delay_start_in_trial) > 1) or (len(delay_end_in_trial) > 1):
                print('error: more than one start time, skip the trial')
                continue
...
def time_to_sequence(spike_time_trial, T, timestep, before=0.5):
    ...
    spike_time_trial: spike times for 1 trial
    T: number of time points
    timestep: time interval between each time point
    before: time before the stimulus onset

    return 
    f: spike train for 1 trial
...
def get_flag_for_hemisphere_and_ALM(units, ids_sort_by_area):
    # is in left brain
    is_left = np.zeros((len(units),))
    is_ALM = np.zeros((len(units),))

    nii = nib.load(DATA_DIR / "mice-MAP" / "ALM_functionalData.nii")
    V = nii.get_fdata()

    # Find nonzero voxels and get their indices
    nonzero_indices = np.argwhere(V != 0)

    # Extract r, c, v from indices (MATLAB [r, c, v] = [AP, DV, ML])
    r, c, v = nonzero_indices[:, 0], nonzero_indices[:, 1], nonzero_indices[:, 2]
    ...
    for ind, id in enumerate(ids_sort_by_area):
        x_ccf = units[units.id==id]['electrodes'].iloc[0].x.values[0]

        if x_ccf > 5700:
            is_left[ind] = 1

        if 'motor' in units[units.id==id].anno_name.iloc[0]:
            y_ccf = units[units.id==id]['electrodes'].iloc[0].y.values[0]
            z_ccf = units[units.id==id]['electrodes'].iloc[0].z.values[0]

            for alm_ind in range(len(r1)):

                if ((x_ccf > v1[alm_ind] - voxel_rd) and (x_ccf < v1[alm_ind] + voxel_rd)) or ((x_ccf > v2[alm_ind] - voxel_rd) and (x_ccf < v2[alm_ind] + voxel_rd)):
                    
                    if (y_ccf > c1[alm_ind] - voxel_rd) and (y_ccf < c1[alm_ind] + voxel_rd) and (z_ccf > r1[alm_ind] - voxel_rd) and (z_ccf < r1[alm_ind] + voxel_rd):
                        is_ALM[ind] = 1
                        #print(units[units.id==id].anno_name.iloc[0])
                        break
    
    return is_left, is_ALM
...
def main_get_spike_trains(nwbfile):
    ...
    spike_data_combine_all: dictionary of spike trains for different trial conditions
    keys are 'hit', 'miss', 'ignore', 'opto_hit', 'opto_miss', 'opto_ignore'
    e.g. spike_data_combine_all['hit']: N x T x K, left and right hit trials combined

    n_trial_all: dictionary of number of trials for different trial conditions
    e.g. n_trial_all['hit']['left']: number of left hit trials

    area_value: list of area value for each unit
    area_value_dict: dictionary of area value for each area
    ...
    area_value_dict = {area: i for i, area in enumerate(sorted_area)}
    area_value = [area_value_dict[area] for area in units.sort_values(by='anno_name').anno_name]

    is_left, is_ALM = get_flag_for_hemisphere_and_ALM(units, ids_sort_by_area)
    ...
    return spike_data_combine_all, n_trial_all, licks_dict, behavior_dict, units, ids_sort_by_area, opto_type_sorted_dict, sorted_area, area_value, area_value_dict, is_left, is_ALM
